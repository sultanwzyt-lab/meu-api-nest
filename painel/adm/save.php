<?php
    ...
    if($cat == "users" || $cat_get == "users") {
        ...
        if($act == "add") {
            pg_query($conn, "INSERT INTO users (username, token, atk, saldo, valorapostado, valordebitado, valorganho, rtp, isinfluencer, agentid) VALUES ('$username', '$token', '$atk', '$saldo', '$valorapostado', '$valordebitado', '$valorganho', '$rtp', '$isinfluencer', '$agentid')");
        } elseif ($act == "edit") {
            pg_query($conn, "UPDATE users SET username = '$username', token = '$token', atk = '$atk', saldo = '$saldo', valorapostado = '$valorapostado', valordebitado = '$valordebitado', valorganho = '$valorganho', rtp = '$rtp', isinfluencer = '$isinfluencer', agentid = '$agentid' WHERE id = '$id'");
